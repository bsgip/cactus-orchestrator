...
@app.post("/run", status_code=HTTPStatus.CREATED)
async def spawn_teststack(test: SpawnTestRequest) -> SpawnTestResponse:
    ...
    except (K8sManagerException, RunnerClientException) as exc:
        logger.debug(exc)
...
@app.post("/run/{run_id}/finalize", status_code=HTTPStatus.OK)
