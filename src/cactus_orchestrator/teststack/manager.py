...
class PodmanTeststackManager:

    def _pod_name(self, teststack_id: str) -> str:
        return get_current_settings().template_service_name_prefix + teststack_id

    def _client(self) -> podman_api.PodmanClient:
        return podman_api.PodmanClient(base_url=f"unix://{get_current_settings().podman_socket}")

    def get_resource_names(self, teststack_id: str) -> TeststackResourceNames:
        settings = get_current_settings()
        pod_name = self._pod_name(teststack_id)
        return TeststackResourceNames(
            runner_base_url=PODMAN_RUNNER_URL.format(pod_name=pod_name, svc_port=settings.podman_runner_port),
            envoy_base_url=TEST_EXECUTION_URL_FORMAT.format(fqdn=settings.test_execution_fqdn, svc_name=pod_name),
        )

    async def spawn(self, teststack_id: str, csip_aus_version: str, user_name: str) -> TeststackResourceNames:
        settings = get_current_settings()
        ...
        pod_name = self._pod_name(teststack_id)
        href_prefix = f"/{pod_name}"
        resource_names = self.get_resource_names(teststack_id)

        try:
            await asyncio.to_thread(self._create_pod_and_containers, pod_name, images, href_prefix, settings)
        except Exception:
            logger.warning(f"Failed to create pod {pod_name}, cleaning up")
            await self.destroy(teststack_id)
            raise

        await self._wait_for_runner_healthy(pod_name)

        logger.info(f"Teststack pod {pod_name} ready for user {user_name}")
        return resource_names

    def _create_pod_and_containers(self, pod_name: str, images: dict[str, str], href_prefix: str, settings) -> None:
        with self._client() as client:
            client.pods.create(name=pod_name, network_mode=settings.podman_network)
            ...
            traefik_labels = {
                ...
                f"traefik.http.services.{pod_name}.loadbalancer.server.port": "8080",
            }
            client.containers.run(
                images["envoy"],
                detach=True,
    ...
    async def _wait_for_runner_healthy(self, pod_name: str) -> None:
        runner_name = f"{pod_name}-runner"
        for attempt in range(POD_READY_MAX_ATTEMPTS):
            try:
                status = await asyncio.to_thread(self._get_container_health, runner_name)
                if status == "healthy":
    ...
    async def destroy(self, teststack_id: str) -> None:
        pod_name = self._pod_name(teststack_id)
        try:
            await asyncio.to_thread(self._destroy_pod, pod_name)
            logger.info(f"Teststack pod {pod_name} destroyed")
        except Exception as exc:
            logger.warning(f"Error destroying pod {pod_name}: {exc}")

    def _destroy_pod(self, pod_name: str) -> None:
        with self._client() as client:
            try:
