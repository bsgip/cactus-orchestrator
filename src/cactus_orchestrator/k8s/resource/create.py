...
async def clone_statefulset(template_names: TemplateResourceNames, run_names: RunResourceNames, user_name: str) -> None:
    ...
    update_containers = [c for c in new_spec.template.spec.containers if c.name == "envoy"]
    if len(update_containers) != 1:
        raise CactusOrchestratorError(
            f"{template_names.namespace} {template_names.stateful_set} - Expected 1 but found {len(update_containers)}"
            + " envoy containers."
        )
